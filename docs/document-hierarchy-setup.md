...
| `create_test_documents.sh <customer_id> <account_id> <application_id>` | Generates a full 4-document sample set for one customer (dummy PDFs), matching the original plan's ingestion mapping. Good for smoke-testing a fresh customer branch end to end. Each document also gets a fresh `unique_ref` UUID (see below). |
| `upload_document.sh <file> <customer_id> <category> [account_id] [application_id]` | Files one **real** file into the hierarchy at whatever level you specify — customer, account, or application — inferring the document type from which IDs you pass. Also attaches a fresh `unique_ref` UUID (see below). |
| `add_unique_ref_metadata.sh` | One-time, run after `setup_document_hierarchy.sh`: adds the `unique_ref` metadata type (see below). Idempotent — safe to re-run against an instance it's already been applied to. |
...
## The `unique_ref` metadata field

A fifth metadata type, `unique_ref`, was added later via
`scripts/add_unique_ref_metadata.sh`. It's an identity field, not a
hierarchy field: unlike `customer_id`/`account_id`/`application_id`/
`category`, it plays **no role in the index tree** — no node expression
references it, so attaching or changing it never affects where a document
files, and doesn't need an index rebuild.

Purpose: a stable per-document unique string (a UUIDv4), independent of
Mayan's own numeric `id` and `uuid` fields, generated by our tooling rather
than by Mayan itself. It's searchable — the webapp's search screen has a
dedicated "Unique Ref" filter (exact match, same mechanism as the other
metadata filters — see `docs/webapp-implementation-plan.md`) — and shown
read-only in the webapp's document preview popup (it's deliberately absent
from the edit-metadata form, so there's no UI path to overwrite it).

`add_unique_ref_metadata.sh` does four things, in order, and is safe to
re-run (skips anything already done):

1. Creates the `unique_ref` metadata type (find-or-create by name).
2. Attaches it to all three hierarchy document types **as optional**.
3. Backfills a UUIDv4 onto every existing hierarchy document that doesn't
   already have one.
4. *Then* flips the field to `required: true` on all three document types
   — only after every existing document already satisfies it, so a fresh
   run never leaves an already-uploaded document in a state that
   contradicts a field marked required.

Going forward, `scripts/upload_document.sh`, `scripts/create_test_documents.sh`,
and the webapp's upload flow (`documents_service.upload_document`) all
generate and attach a `unique_ref` UUID automatically at creation time —
you don't need to (and normally shouldn't) set it by hand.

**Uniqueness is generated, not enforced.** Nothing in Mayan or this
tooling rejects a duplicate `unique_ref` value — `uuid.uuid4()`'s
collision odds are treated as good enough. If you ever hand-edit this
field via the API, you're responsible for not colliding.

See also Gotcha #5 below: marking it `required` did **not** turn out to
block API-driven document creation the way you'd expect.
...
3. Open the uploaded document's **Metadata** tab (you may be prompted for
   ...
   - `unique_ref` — required per the document type config, but the web UI
     has no way to generate one for you. Generate a UUID yourself (e.g.
     `python3 -c "import uuid; print(uuid.uuid4())"` or `uuidgen`) and
     paste it in. This manual step is the one gap the scripts/webapp
     upload paths close automatically — see "The `unique_ref` metadata
     field" above.
   Save.
...
## Gotcha #5: `required: true` on a document-type metadata field isn't enforced by the API

Marking `unique_ref` `required` on the three hierarchy document types (via
`PATCH /document_types/<id>/metadata_types/<relation_id>/`) does **not**
stop `POST /documents/` from creating a document of that type with no
`unique_ref` attached — verified directly: creating a bare `Customer
Document` with only `document_type_id` and `label` still returns `201`.
`required` only drives validation in **Mayan's own web upload form** (it
prompts you for the field after upload); it's not a database or
serializer-level constraint the REST API enforces.

Net effect: nothing blocks a script or API client that forgets to attach
`unique_ref` — same class of silent-acceptance gotcha as Gotcha #3
(`action_name`). This is *why* `scripts/upload_document.sh`,
`scripts/create_test_documents.sh`, and the webapp's upload flow all
generate `unique_ref` themselves rather than relying on the `required`
flag to catch a missing one.
...
All documents above also carry a `unique_ref` (backfilled via
`add_unique_ref_metadata.sh`) — omitted from this table since the values
are random UUIDs with no mnemonic meaning; check a given document's
Metadata tab or `GET /documents/<id>/metadata/` for its actual value.
